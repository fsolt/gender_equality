...
There are likely to be, among those reading these pages, some who will find some of the conclusions of the two previous chapters not entirely believable, in particular those regarding public responsiveness.
Recall that, across the countries of the OECD over several decades, we found evidence for not only democratic representation, politics and policy reflecting public opinion, but also _positive_ public responsiveness to politics and policy, that is, that public opinion responds to women's electoral success and the adoption of more gender egalitarian policies by growing more egalitarian.
Such accelerating feedback is the exact opposite of what many observers fear is true, that moves toward gender equality prompt backlashes in public opinion.

Korea's recent political history provides a strong basis for these fears of public opinion backlash.
...
At least on its face, this sequence of events appears to stand in contrast to our conclusions.
...
2022 election again very close

## A Backlash Against Gender Equality?
...
caption <- "The pale shaded region indicate the years of Korea's first woman president, Park Geun-hye, from her election in 2013 to her impeachment and removal for corruption in 2018. Darker shading indicates the 80% credible interval of the PGE scores."
...
pge_summary %>% 
  ...
  geom_ribbon(aes(ymin = p10, ymax = p90,
              ...
              alpha = .2) +
  geom_line() +
  geom_text(data = tibble(year = 2015.5,
                          pge = 96),
            label = "Park\nAdministration",
            size = 4) +
...
The PGE scores provide no clear evidence of a decline in gender egalitarian attitudes during these years.
When Park started her term in 2013, the country had a PGE score of 50.1 (80% credible interval: 46.2 to 52.2); when Park was removed from office in 2018, its PGE score was 51.6 (80% c.i.: 49.9 to 53.3).

But neither do these data show clear evidence of the _increase_ in macroegalitarianism that the cross-national results on women's officeholding we presented in Chapter 4 would lead us to expect to occur during a time when the highest political office in the country was held by a woman. 
While estimated to be positive, the difference in PGE scores over these years---1.5 points---is not credibly different from zero (80% c.i.: -2.1 to 5.1).
...
women_execs0 <- rvest::read_html("https://en.wikipedia.org/wiki/List_of_elected_and_appointed_female_heads_of_state_and_government") %>% 
  rvest::html_table(fill=TRUE) %>% 
  first() %>% 
  janitor::clean_names() %>% 
  filter(str_detect(executiveor_non_executive, "Exec") &
           country %in% oecd_countries) %>% 
  mutate(mandateend = if_else(mandateend == "Incumbent",
                              "4 August 2025",
                              mandateend) %>% 
           str_remove("Suspended.*"),
  ...
  mutate(name = iconv(name, "UTF-8", "UTF-8"),
         ...
         t_val = diff/diff_se,
         p_val = pnorm(t_val, lower.tail = FALSE),
...
women_execs_long0 <- women_execs %>% 
  ...
  left_join(pge_summary %>% 
              select(country, year, pge, pge_se, p10, p90),
            by = join_by(country, year)) %>% 
  filter(!is.na(pge)) %>% 
  group_by(country, name2) %>% 
  mutate(n = n(),
         diff = pge - first(pge),
         diff_se = sqrt(pge_se^2 + first(pge_se)^2)/2) %>% 
  left_join(eapq0 %>% 
...
women_execs_long1 <- women_execs_long0 %>% 
  group_by(name2, year) %>% 
  mutate(full_year = n() == 4) %>% 
  ungroup() %>% 
  left_join(eap %>% 
              transmute(country = country_name,
                     year, 
                     approval_annual = Approval_Not_Smoothed,
                     disapproval_annual = Disapproval_Not_Smoothed,
                     approval_annual_net = approval_annual -
                       disapproval_annual) %>% 
              filter(!is.na(approval_annual_net) &
                       approval_annual + disapproval_annual < 100),
            by = c("country", "year")) %>% 
  mutate(approval_net = if_else(full_year &
                                  !is.na(approval_annual) &
                                  !is.na(disapproval_annual),
                                approval_annual - disapproval_annual,
                                approval_net))

```

```{r, fig.cap="Trends in Macroegalitarianism in OECD Countries That Have Had Women Chief Executives", fig.height=4}
...
caption <- "The pale shaded regions indicate the years during which women served as chief executives in these countries. Darker shading indicates the 80% credible interval of the PGE scores."

pge_summary %>% 
  filter(country %in% women_execs$country &
           !country == "South Korea") %>%
  ggplot() +
  ...
  geom_ribbon(aes(x = year, ymin = p10, ymax = p90), 
              alpha = .2) +
  facet_wrap(. ~ factor(country, levels = rev(unique(women_execs$country))),
             ncol = 4) +
  labs(x = NULL, y = "PGE Score") +
  theme_bw() +
...
```{r trends_women_execs}
# saved and loaded because some characters in leaders' names 
# don't render correctly through pandoc

caption = "Note: Leaders' terms are ordered by the change in PGE scores over the years in office; whiskers represent 80% credible intervals."

trends_women_execs <- women_execs_long1 %>%
    group_by(name2) %>% 
    summarize(pge_change = last(pge) - first(pge),
              pge_change_se = sqrt(last(pge_se)^2 + first(pge_se)^2),
              name3 = paste0(first(name2 %>% 
                                     str_remove("^\\S+( Harlem)?") %>% 
  ...
  mutate(name3 = as_factor(name3) %>% 
           fct_reorder(-pge_change)) %>% 
  ggplot(aes(x = pge_change, y = name3)) +
  ...
  geom_point(fill = "black", shape = 21, size = .5, na.rm = TRUE) +
    ...
    ggtitle("Change in PGE,\nFirst to Last Year of Term") +
  plot_annotation(caption = str_wrap(caption, 90))

ggsave(here("paper", "korea", "data", "trends_women_execs.png"),
       trends_women_execs,
       width = 6.5,
       height = 4.5)
```

```{r, fig.cap="Changes in Macroegalitarianism Over the Terms of OECD Woman Chief Executives", cache=FALSE}
#| label: fig-trends_women_execs

# saved and loaded because some characters in leaders' names 
# don't render correctly through pandoc

knitr::include_graphics(here("paper",
                             "korea",
                             "data",
                             "trends_women_execs.png"),
                        dpi = 400)

```

@fig-trends_women_execs shows how attitudes toward gender equality in the public sphere changed over the terms in office of women chief executives in the OECD for which we have PGE scores.
During these 28 terms in office, the change is estimated to be positive---that is, toward more macroegalitarianism---more than twice as often as it is negative.
Calculating the difference between two quantities measured with uncertainty results in even more uncertainty, however, so only rarely are these positive changes credible with 80% confidence.
...
caption <- "All survey items are coded so that more egalitarian responses have higher values. The shaded region indicates the years of Korea's first woman president, Park Geun-hye, from her election in 2013 to her impeachment and removal for corruption in 2018."
...
Our focus on Korea in this chapter allows us to look more closely than we could when we took the broad comparative vantage that we adopted earlier in the book.
...
The years that Park held the presidency, 2013--2018, are again shaded in pale gray.
...
Before exploring further, it is worth pausing a moment to remember that this pattern is distinctly unusual.
We found in Chapter 3 that items about equality in politics and in the workplace are typically tightly bound to each other (see, for example, @fig-pge_pwe_corr_plot).
That they are not in this case suggests that something interesting is going on.

### By Item, Cross-National
...
```{r, fig.cap="Trends in Mean Survey Responses on Gender Equality in Politics in OECD Countries That Have Had Women Chief Executives", fig.height = 9, fig.width = 3.75, eval=FALSE}
...
# 
# terms <- surveys_oecd_political %>% 
#   select(country, ends_with("_yq")) %>% 
#   distinct()
# 
# poli_plots <- surveys_oecd_political %>% 
#   split(surveys_oecd_political$item) %>% 
#   map(~.x %>% 
...
kd1_means %>%
  ...
  scale_color_manual(values = c("Men" = "grey70",
                                ...
                                "Uni" = "grey30"),
                     aesthetics = c("colour", "fill"))  +
  plot_annotation(caption = str_wrap("Shaded region indicates the years of Korea's first woman president, Park Geun-hye, from her election in 2013 to her impeachment and removal for corruption in 2018", 120))
...
pge_eap1 <- pge_eap %>% 
  list_rbind() %>% 
  group_by(country, name2) %>% 
  summarize(pge_diff_se = sd(pge_diff),
...
pge_eap_plot <- pge_eap1 %>% 
  ggplot(aes(y = pge_diff,
             x = approval_net_mean)) +
  geom_smooth(method = 'lm', se = FALSE) +
  geom_text(aes(label = name2),
            size = 2.5) +
  geom_segment(aes(y = pge_diff - 1.28*pge_diff_se,
               yend = pge_diff + 1.28*pge_diff_se),
               alpha = .2) +
  theme_bw() +
...
ggsave(here("paper", "korea", "data", "pge_eap_plot.png"),
       pge_eap_plot,
       width = 6.5,
       height = 8.25)
```

```{r, fig.cap="Macroegalitarianism and Executive Approval by Term for Woman Chief Executives", cache=FALSE}
#| label: fig-pge_eap
...
Even with the large uncertainties in the change in PGE scores and the small number of observations---executive approval data is only available for 23 terms of women chief executives in the Executive Approval Project dataset---the positive relationship is still evident.
