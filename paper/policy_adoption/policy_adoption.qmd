...
Belgium and Costa Rica adopted their 50% requirements in two steps, previously requiring 33% and 40% of parties' candidates to be women respectively, while Mexico took three, with prior stops at 30% and 40%.
...
Does macroegalitarianism help us to explain these differences?
...
As before, we rely on the Feminist Movement Index [@Forester2022] as our measure of the strength of these movements (see Figure 4.5).
...
A country with mean PGE scores two standard deviations higher, for example New Zealand compared to Poland, is estimated to enact quotas requiring `r get_coef("pge_mean", coef_data_m1_nq)` percentage points more women candidates.
...
Or do quotas work to help make gender equality salient [see @Weeks2022] and so increase macroegalitarianism?
...
As in the previous chapter we model the potentially reciprocal relationships between our two variables of greatest interest using two equations, the first for quota adoption and reform and the second for macroegalitarianism, that are estimated within a multivariate simultaneous equations model.
...
It provides additional support for the conclusions we drew from the findings presented in @fig-m1-nq-plot: macroegalitarianism predicts national quota adoption and reform in both the short term, as evidenced by the estimate for its difference, and the long term, as evidenced by the estimate for its mean.
...
Increases in feminist movement strength appear to prompt higher quotas, and countries with more experience with list electoral systems are more likely to adopt quotas as well.
...
This is reassuring, but it is really the right side of the figure that is our current focus.
...
Before we reach that answer, however, we again start from the bottom with the estimates for our control variables.

After the estimate of the dynamic effect of past values macroegalitarianism on present values in the bottom right panel, the two bottom rows of the top right panel present the estimates for GDP per capita.
...
Countries with higher GDP per capita exhibit more macroegalitarianism: compared to a country that is a standard deviation below the mean on this variable, a country that is a standard deviation above is predicted to have a PGE score `r get_coef("gdppc_mean", coef_data_nqsem)` points higher, compounding over time in this dynamic model to `get_coef("gdppc_mean", coef_data_nqsem_total)` points.
...
A two-standard-deviation gain in GDP per capita from one year to the next is estimated to yield an increase of `get_coef("gdppc_diff", coef_data_nqsem)` points in a country's PGE score.
This increase then grows to a total of `get_coef("gdppc_diff", coef_data_nqsem_total)` over time.

Next are the estimates for electoral systems with a party-list component.
As also seen in @fig-m3-wr in the last chapter, net of the other variables in the model, countries that elect at least some legislators via a party list are estimated to have less macroegalitarianism than those with only more candidate-oriented small and single-member districts.
In this model, this difference in PGE scores is estimated as `r coef_data_nqsem %>% filter(.variable == "b_pge_list_mean" & .width == .8) %>% glue_data("{std_coef} (80% c.i. {ci})") %>% str_remove_all("-")` points, growing over time to `r coef_data_nqsem_total %>% filter(.variable == "b_pge_list_mean" & .width == .8) %>% glue_data("{std_coef} (80% c.i. {ci})") %>% str_remove_all("-")`.
Whether this is a true long-run effect of list electoral systems or instead the result of an omitted variable is unclear.
The latter interpretation is perhaps reinforced by the inconclusive posterior distribution for differences over time in such electoral systems, although the width of that distribution is at least partially the product of the near absence in our dataset of changes in this variable within countries. 
...
Neither cross-country differences nor within-country year-to-year changes yield estimates that can be distinguished from zero with much if any confidence.

As we have seen previously, the same cannot be said for women's descriptive representation.
Countries that have on average elected more women over the timeframe examined here exhibit higher levels of macroegalitarianism than those that have elected fewer women, consistent with a long-term effect.
...
Thanks to strong influence of the past on present macroegalitarianism, this jump up is estimated to grow over time to `r coef_data_nqsem_total %>% filter(.variable == "b_pge_women_rep_mean" & .width == .8) %>% glue_data("{std_coef} (80% c.i. {ci})")` points.
Similarly, a two-standard-deviation increase in descriptive representation over time is estimated to correspond to `r coef_data_nqsem %>% filter(.variable == "b_pge_women_rep_diff" & .width == .8) %>% glue_data("{std_coef} (80% c.i. {ci})")` point gain in macroegalitarianism, which increases over time to `r coef_data_nqsem_total %>% filter(.variable == "b_pge_women_rep_diff" & .width == .8) %>% glue_data("{std_coef} (80% c.i. {ci})")` points.
