...
Party quotas, on the other hand, are adopted voluntarily by parties: they constitute a promise by the adopting party that women will represent at least the specified share of the candidates the party puts forward for office.
As we saw in the previous chapter, both legislated national quotas and voluntary party quotas have proven effective policies for increasing the share of women in office.
Given our theory that public opinion matters not 
...
```{r quota_m2}
if (!file.exists(file.path(policy_path,  "data", "quota_m2_results.rda"))) {
...
```{r m1_q_plot, fig.cap="Predicting Gender Quotas in the OECD \\label{m1_q_plot}", fig.height = 7, fig.width = 7.5}
