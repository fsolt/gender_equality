...
```{r m2nqplot, fig.cap="Predicting the National Quota Adoption and Macroegalitarianism in OECD Countries", fig.height = 9, fig.width = 7.5}
#| label: fig-m2-nq
...
```{r wblplot, fig.cap="Macroegalitarianism and National Quotas in the OECD", fig.height = 7, fig.width = 7.5}
#| label: fig-wbl-corr
pge <- rio::import(here("data",
                        "pge.rda"))
...
As we have seen in @fig-m3-wr, gains in women's descriptive representation also yield higher macroegalitarianism in the democracies of the OECD.
...
The results are displayed in @fig-m1-wbl-plot.

`r get_coef("pge_mean", coef_data_m1)` 
`r get_coef("pge_mean", coef_data_m1_total)`
...
@fig-m2-wbl-plot depicts the results.


### A Further Test: Childcare Provision

One known shortcoming of the longitudinal WBL data we use here is its exclusion of the single policy most closely tied to gender equality in the workforce: childcare.
...
**Availability is more important than spending, and the available spending data has validity problems anyway because the it covers older preschool children, whose care is associated more with part-time, low-wage, dead-end work.**
To measure childcare availability across our countries of interest, we look to OECD data on enrollments in early childhood education and care among the very young, children aged zero to two. **Cite**
@fig-enrplot shows how enrollments have varied across countries and over time.
