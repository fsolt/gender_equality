...
Some countries, such as Belgium, Costa Rica, and Mexico, now have quotas that require parity in the number of women and men each party puts forward as candidates (see again Figure 4.4 in the previous chapter).^[
The terms of France's trailblazing "parity" quota, adopted in 1999, permits parties to deviate from the quota by up to 2% without penalty
]
...
Still other countries, many in fact, have no legislated and mandatory gender quota at all.
We therefore treat quota adoption as a continuous rather than dichotomous variable [see @Boehmke2009, 237].
We also consider the adoption of a parity quota, one that requires 50% of a party's candidates to be women, to be a fully gender egalitarian policy adoption.
This implies that a country with a higher quota has less 'exposure' to the 'risk' of requiring parties to run additional women candidates than a country with a lower quota or no quota at all; exposure is included in the model as the log of the difference between 50% and the current quota [see @Boehmke2009, 238].
Our treatment of parity as the maximum quota also means that a country leaves the risk set entirely---further observations of the country are excluded from the data---after it has adopted a parity quota.
...
by_country3 <- by_country2 %>% 
  mutate(exposure = log1p(50-lag(national_quota)),
         prev_quota = as.numeric(!lag(national_quota)==0)) %>% 
  filter(lag(national_quota < 50))
...
The bivariate relationship between the PGE scores and national quotas among the observations within the risk set, depicted in @fig-nq-plot, is rather weak.
Many countries, even those whose publics hold relatively egalitarian views regarding gender roles in the public sphere, have never adopted national gender quotas for legislative candidates.
...
```{r enrplot, fig.cap="Enrollment in Early Childhood Education and Care in the OECD \\label{fmiplot}", fig.height = 9, fig.width = 7.5}
ecec_data1 %>%
  group_by(country) %>%
