# From Public Egalitarianism to Advancing Equality: Explaining Policy Adoption
...
## Advancing Gender Equality in Politics {.unnumbered}
...
by_country3 <- by_country2 %>%
  group_by(country) %>% 
  mutate(quota_lag = if_else(!is.na(lag(national_quota)), 
                             lag(national_quota),
                             national_quota),
         quota_change = national_quota-quota_lag,
...
The bivariate relationship between the PGE scores and national legislative quotas in our data, depicted in @fig-nq-plot, is at best weak.
...
<!-- That is, the influential typology presented by @Dahlerup2005 is incomplete: legislative quotas are not simply adopted to provide a 'fast track' to women's descriptive representation or rejected as unnecessary and undesirable in an 'incremental track.' -->
...
We use as our unit of analysis the country-term, as our predictors vary little from year to year between legislative elections; these variables are measured at the beginning of the term.
...
Recall too that there are other important aspects of national legislative quotas, such as whether and how they are enforced or if the position of women on an electoral list is specified [see, e.g., @Piscopo2024], but we leave these characteristics aside in this analysis.]
...
We therefore treat quota adoption as a continuous rather than dichotomous variable [see @Boehmke2009, 237], measured as the additional percentage.
...
However, the general EHA approach is flexible [see @Boehmke2009].
This flexibility is important firstly, as @Piscopo2024 explains and as Figure 4.4 in the previous chapter illustrates, many countries have not simply adopted quotas requiring a specified share of the candidates each party puts forward to be women but have also raised this share, sometimes repeatedly, to reach or at least more closely approach parity.
Removing a country from the analysis after its first quota adoption, as in the standard EHA approach described above, would prevent us from analyzing the 'steady path' by which quotas are sometimes reformed.
We consider the adoption of a parity quota, one that requires 50% of a party's candidates across all seats to be women, to be the quota policy that maximally advances gender equality.
This implies that a country with a higher quota has less 'exposure' to the 'risk' of requiring parties to run additional women candidates than a country with a lower quota or no quota at all; exposure is included in our model as the log of the difference between an 50% and the current quota [see @Boehmke2009, 238].
Our treatment of parity as the quota that most advances gender equality also means that a country leaves the risk set entirely---and so further observations of the country are excluded from the data---only after it has adopted a parity quota.

This flexibility is important secondly because national legislative gender quotas and reforms are not all the same, either present or absent.
Some countries---France, Belgium, Costa Rica, and Mexico---now have quotas that require parity in the number of women and men each party puts forward as candidates across all legislative seats.
Other countries have adopted quotas that, although requiring parties to nominate _some_ women candidates, do not require parties to run equal numbers of women and men.
Still other countries, many in fact, have no mandatory legislative gender quota at all.
...
To measure the adoption and reform of quotas we again employ the data on the de facto threshold provided by the excellent QAROT database [@Hughes2019] updated with information from @IDEA2023, but now taking as our variable of interest the additional percentage of candidates who are mandated to be women after the policy's enactment, with reforms _reducing_ the number of women candidates parties are required to field represented as negative values.^[
...
if (!file.exists(file.path(policy_path,  "data", "m1_nq_results.rda"))) {
  bf_national_quota <- bf(quota_change ~
...
## Advancing Gender Equality in the Workplace {.unnumbered}
...
```{r m2wblplot, fig.cap="Predicting the WBL Policy Index and Public Gender Egalitarianism in OECD Countries \\label{m2_wbl_plot}", fig.height = 9, fig.width = 7.5}
...
@Kittilson2008: descriptive representation boosts maternity and childcare leave; results for other predictors are inconsistent
...
```{r leave_data}
if (!file.exists(file.path(policy_path, "data-raw", "oecd_pf2_5.xlsx"))) {
  download.file("https://web.archive.org/web/20240718100341/http://www.oecd.org/els/family/PF2_5_Trends_in_leave_entitlements_around_childbirth.xlsx",
              file.path(policy_path,
                           "data-raw",
                           "oecd_pf2_5.xlsx"))
}

leave_vars <- read_excel(file.path(policy_path, "data-raw", "oecd_pf2_5.xlsx"),
                         sheet = "Time-Series") %>% 
  names() %>% 
  tolower() %>% 
  str_subset("[a-z].*") %>% 
...
leave_data <- read_excel(file.path(policy_path, "data-raw", "oecd_pf2_5.xlsx"),
                         sheet = "Time-Series",
                         skip = 8,
                         col_names = leave_vars) %>% 
  mutate(country = countrycode(country, "country.name", "country.name"),
         across(-country, as.numeric), 
         equality_leave = total_paid + 
           total_father_specific_paid -
           maternity_weeks)
...
if (!file.exists(file.path(policy_path, "data-raw", "leave.rda"))) {
  ...
  leave_data1 <- pge_summary %>% 
    select(country, year, pge, pge_se) %>% 
    right_join(leave_data,
              by = c("country", "year")) %>%
    left_join(qarot,
              by = c("country" = "countryname", "year")) %>% 
    left_join(des,
    ...
    mutate(countryname = country,
           ...
           pge_se_lag = lag(pge_se),
           fmi = if_else(country == "United States" & year < 1975, 3, fmi),
           equality_leave_included = ifelse(!is.na(lag(pge)),
                                            equality_leave,
                                            NA_real_),
           equality_leave_reform = equality_leave - lag(equality_leave),
           national_quota = if_else(is.na(defacto_threshold), 0,
                                    defacto_threshold)) %>% 
    left_join(gdppc, by = c("country", "year")) %>% 
    mutate(gdppc = gdppc/10000) %>% 
  ...
  save(leave_data1, file = file.path(policy_path, 
                                     "data-raw",
                                     "leave_data1.rda"))
} else {
  pge_summary <- rio::import(here("data",
  ...
  leave_data1 <- rio::import(file.path(policy_path,
                                       "data-raw", 
                                       "leave_data1.rda"))
}
```

```{r leaveplot, fig.cap="Gender Equality Leave in the OECD", fig.height = 9, fig.width = 7.5}
#| label: fig-leaveplot

leave_data1 %>%
  group_by(country) %>%
  mutate(last_leave = last(equality_leave, na_rm = TRUE)) %>% 
  ggplot(aes(x = year, y = equality_leave)) +
  geom_line(linetype = "dotted") +
  geom_line(aes(y = equality_leave_included)) +
  theme_bw() +
  theme(legend.position = "none") +
  # coord_cartesian(xlim = c(1999, 2022)) +
  labs(x = NULL, y = "Gender Equality Leave") +
  facet_wrap(~reorder(country, -last_leave), ncol = 6) +
  theme(axis.text.x  = element_text(size = 7,
                                    angle = 90,
                                    vjust = .45,
                                    hjust = .95),
...
if (!file.exists(file.path(policy_path,
  ...
  leave_data2 <- leave_data1 %>% 
    filter(!is.na(equality_leave_reform)) %>% 
    group_by(country) %>% 
    transmute(year,
              equality_leave_reform,
              equality_leave_lag = lag(equality_leave), # previous year
              equality_leave_mean = mean(equality_leave_lag, na.rm = TRUE),
              equality_leave_diff = equality_leave_lag - equality_leave_mean,
              equality_leave_reform_count = cumsum(!equality_leave_reform==0),
              women_rep,
              women_rep_lag = lag(women_rep), # previous year
              women_rep_mean = mean(women_rep, na.rm = TRUE),
              women_rep_diff = women_rep - women_rep_mean,
  ...
  bf_leave <- bf(equality_leave_reform ~ 
                 me(pge_diff, pge_diff_se) +
                 me(pge_mean, pge_mean_se) +
                 women_rep_diff +
                 women_rep_mean +
                 ...
                 gdppc_diff +
                 gdppc_mean +
                 equality_leave_reform_count +
                 (1|c|country) + (1|t|year))

  m1_leave_b <- brm(formula = bf_leave,  
                  data = leave_data2,
                  backend = "cmdstanr",
                  control=list(adapt_delta = 0.99, 
                               max_treedepth = 14),
...
As described previously, we use data from QAROT and @IDEA2023 for the former and from @Forrester2022 for the latter.
