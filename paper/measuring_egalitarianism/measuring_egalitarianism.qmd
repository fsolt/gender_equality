...
```{r dcpo_input_raw_pge, include=FALSE, cache = TRUE, cache.extra = tools::md5sum(here("paper", "measuring_egalitarianism", "data-raw", "surveys_pge_classification.csv"))}
...
```{r dcpo_input_raw_balance, include=FALSE, cache = TRUE}
...
```{r egr_summary_stats, cache = TRUE, cache.extra = tools::md5sum(here("paper", "measuring_egalitarianism", "data", "dcpo_input_raw_public.csv"))}
...
save(theta_summary, file = here("data",
                                "theta_summary.rda"))
```
...
countries <- c("Norway", "Australia", "United States", "Argentina",
               "Mexico", "Poland", "South Africa", "Japan",
...
ggplot(data = c_res, aes(x = year, y = mean)) +
  ...
  geom_ribbon(data = c_res, 
              aes(ymin = q10, ymax = q90, linetype=NA),
              alpha = .25) +
...
save(pge, file = here("paper", "measuring_egalitarianism", "data", "pge.rda"))
```
