...
Next, we will examine this variation more closely in each of the world's regions.
...
```{r eapac_ts, fig.cap="PGE Scores in East Asia and the Pacific Over Time \\label{eapac_ts}", fig.height = 5}
...
The public in both Cambodia and Indonesia registered a statistically discernable decline in gender egalitarianism.
...
The available survey data described in the first section of this chapter allows us to estimate public gender egalitarianism in sixteen countries across the Middle East and North Africa.
The countries of East Asia and the Pacific exhibit more variation than the countries of either of the two regions discussed above.
Figure&nbsp;\ref{eapac_map} shows the most recent PGE scores for each territory in the region.
In New Zealand and Australia, PGE scores exceed .75 in the most recent year available. 
On the other hand, attitudes remain decidedly opposed in gender equality in politics and the workplace in Myanmar.
...
```{r mena_map, fig.cap="PGE Scores Across East Asia and the Pacific, Most Recent Available Year \\label{eapac_map}", fig.height = 9, cache=FALSE}
region_maps[[4]] +
  theme(legend.position = c(.01, .01),
        legend.justification = c(0, 0),
...
```{r mena_ts, fig.cap="PGE Scores in East Asia and the Pacific Over Time \\label{mena_ts}", fig.height = 5}
