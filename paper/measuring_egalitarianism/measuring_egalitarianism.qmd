...
# Measuring Public Gender Egalitarianism Around the World
...
The PGE data are an invaluable source for studying the consequences of collective attitudes toward gender equality in the public sphere over time in countries around the world.
...
## Examining the Source Data on Public Gender Egalitarianism {.unnumbered}
...
The complete list of public gender egalitarianism survey items is included in the online supplementary materials Appendix A.  A discussion of excluded survey items---those covering the related but distinct concepts of gender equality in the private sphere, how women should balance opportunities in the public sphere with their traditional duties in the private sphere, or issues of gender dominance such as sexual harassment---is found in online Appendix B.]
...
## Estimating Public Gender Egalitarianism {.unnumbered}
...
## Public Gender Egalitarianism Around the World {.unnumbered}
Figure&nbsp;\ref{cs_mry} and Figure&nbsp;\ref{world_map} display the most recent available PGE score for each of the `r res_c` countries and territories in the dataset.
Together, they underscore the geographic breadth of the PGE dataset, which allows the study of countries and regions too often neglected in political science research [see @Wilson2021].
...
regions_bbox <- map_df(regions_geom,
    ...
    mutate(reg_xmin = case_when(world_region == "Europe" ~ -1700000,
           ...
           reg_ymin = case_when(world_region == "Europe" ~ 4250000,
                                TRUE ~ reg_ymin),
           legend_pos = case_when(world_region == "Europe" ~ "c(.99, .99)",
                                  world_region == "Central Asia" ~ "bottom",
                                  TRUE ~ "right"),
           legend_just = case_when(world_region == "Europe" ~ "c(1, 1)",
                                  world_region == "Central Asia" ~ "bottom",
                                  TRUE ~ "right"))
...
Figure&nbsp;\ref{europe_map} depicts how the most recent PGE score for each country varies across the region.
...
It should be noted, however, that the most recent available PGE scores in even these latter countries still attained or approached .5, indicating that roughly half the population holds egalitarian views. 
...
The PGE scores in each of those countries increased by 30% or more of the measure's theoretical range.
...
\pagebreak
\newpage
### Latin America and the Caribbean {.unnumbered}
...
Figure&nbsp;\ref{latam_map} shows the most recent PGE scores for the region.
...
\pagebreak
### East Asia and the Pacific {.unnumbered}
...
Figure&nbsp;\ref{eapac_map} shows the most recent PGE scores for each territory in the region.
In New Zealand and Australia, PGE scores exceed .75 in the most recent year available. 
...
The public in both Cambodia and Indonesia registered a statistically discernible decline in gender egalitarianism.

\pagebreak
\newpage
### The Middle East and North Africa {.unnumbered}
...
The most recent PGE scores that resulted in these countries are mapped in Figure&nbsp;\ref{mena_map}.
...
Moreover, the premier survey of the region, the Afrobarometer, unfortunately does not include questions regarding gender equality in politics and work.
...
Figure&nbsp;\ref{ssa_map} maps the most recent PGE scores of these countries.
...
The most recent year of PGE estimates available in the region is mapped in Figure&nbsp;\ref{casia_map}.
...
The point estimate of the most recent PGE score, that is, putting aside uncertainty, is scarcely above .2 in that country.
...
### Trends in Public Gender Egalitarianism {.unnumbered}

```{r region_trends}
region_trends_model <- region_pge_list %>% 
  furrr::future_imap(\(reg, reg_name) {
    lmer(pge ~ year +
           I(year^2) +
           (1 | country),
         data = reg) %>% 
      broom.mixed::tidy() %>% 
      filter(effect == "fixed") %>% 
      mutate(model = first(reg$world_region))
  }) %>% 
  list_rbind()
```
...
In the following chapters, we will use the PGE data to better address such long-standing questions as how collective attitudes on gender roles have influenced the election of women to national legislatures [see, e.g., @Paxton2003; @Alexander2012] and to pursue both new and more nuanced lines of inquiry on issues of policy responsiveness [cf., @Kittilson2008; @Busemeyer2021].
