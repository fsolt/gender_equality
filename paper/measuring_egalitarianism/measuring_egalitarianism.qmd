# Measuring the Public's Gender Egalitarianism Around the World
...
```{r dcpo_results_summary, cache=TRUE, cache.extra = tools::md5sum(here("paper", "measuring_egalitarianism", "data", "dcpo_output.rda"))}
...
```{r world_map, fig.cap="PGE Scores Around the World \\label{world_map}", fig.height = 4.5}
world_map_data <- map_data("world") %>%
  filter(!long > 180)

countries_map <- world_map_data %>%
  distinct(region) %>%
  mutate(country = countrycode::countrycode(region,
                                            "country.name",
                                            "country.name"),
         world_region = countrycode::countrycode(country,
                                                 "country.name",
                                                 "region")) %>%
...
countries_map %>%
  ggplot(aes(fill = pge, map_id = region, color = 'NA')) +
  geom_map(map = world_map_data) +
  ...
  scale_color_manual(name = "", 
                     values = 'black',
                     labels = 'No Data') +
  guides(color = guide_legend(override.aes = list(fill = na_value))) +
  theme_map() +
  theme(legend.position="bottom")

```

The Scandinavian countries and Germany are at the top of this list, along with Puerto Rico, which has had women of both of its major parties serve as chief executive and as recently as 2020 had a woman from each party holding the two most prominent elected offices on the island.
...
region_pge <- pge_summary %>%
  ...
  summarize(regional_pge = max(pge, na.rm = TRUE))

region_limits <- tribble(~region, ~lat_lim, ~long_lim,
                         "Europe")
region_limits <- map_df(regions_map_list,
                        \(reg)
                        reg %>% 
                          summarize(world_region = first(world_region),
                                    lat_min = min(lat),
                                    lat_max = max(lat),
                                    long_min = min(long),
                                    long_max = max(long)))

regions_map_list <- world_map_data %>% 
  mutate(country = countrycode::countrycode(region,
                                            "country.name",
                                            "country.name"),
         world_region = countrycode(country,
                                    ...
                                    "region8",
                                    custom_dict = region_dict),
         world_region = ifelse(country == "Russia" &
                                 long < 28,
                               "Europe",
                               world_region)) %>%
  filter(!country == "Greenland" & !region == "Antarctica") %>% 
  left_join(pge_summary %>%
              group_by(country) %>%
              top_n(1, year) %>%
              ungroup() %>%
  ...
  mutate(world_region = fct_reorder(world_region,
                                    -regional_pge,
                                    .na_rm = FALSE)) %>% 
  filter(!(str_detect(world_region, "Pacific") &
             long < 0) &
           !(world_region == "North America" & 
               long > 0)) %>% 
  split(., .$world_region)

region_maps <- map(regions_map_list,
                    \(list) {
                      list %>% 
                        ggplot(aes(fill = pge, map_id = region, color = 'NA')) +
                        geom_map(map = list) +
                        expand_limits(x = list$long,
                          y = list$lat) +
                        coord_map("mollweide") +
                        scale_fill_gradient(name = "Public\nGender\nEgalitarianism",
                                            low = "grey95",
...
```{r region_countries_pge, fig.cap="PGE Scores Over Time Within Selected Countries \\label{ts}", fig.height=3.5}
region_countries_pge <- map(regions_map_list,
                        \(reg) {
                          reg %>%
                            filter(!is.na(pge)) %>% 
                            select(country) %>% 
                            distinct() %>% 
                            left_join(pge_summary, by = "country") %>% 
...
ts_plot <- function(data, ...) {
  ggplot(data = data, aes(x = year, y = pge)) +
    ...
    geom_line(data = data) +
    facet_wrap(~country, ...) +
    force_panelsizes(rows = unit(1.75, "in"), 
                     cols = unit(1, "in"), 
                     TRUE) +
...
The figure also shows that while public opinion favoring gender equality in the public sphere has risen steadily in some countries, such as Norway and Australia, attitudes have changed little over time in others, like China and the Philippines, or fallen, as in Indonesia. 
...
```{r convergent_setup, cache = TRUE, cache.extra = tools::md5sum(here("paper", "measuring_egalitarianism", "data", "pge.rda"))}
...
internal_dat <- dcpo_input$data %>% 
  ...
  left_join(pge %>% select(-kk, -tt), by = c("country", "year"))
