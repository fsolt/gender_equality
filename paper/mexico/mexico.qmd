...
censo_regions <- tibble(state = c("Aguascalientes", "Baja California", 
...
amb_regions <- tibble(var_state = 1:32,
...
for (i in seq(nrow(vars_table))) {
  ...
  vars1 <- t_data %>% 
    ...
    bind_cols(t_data1) %>%
    left_join(amb_regions2, by = join_by(var_state))
...
poststrat_df <- read_feather(file.path(mex20_dir,
  ...
  transmute(year, # numeric
            ...
            stratum, # factor
            state = factor(state),
            var_indig = indig, # logical
            var_female = female, # numeric
