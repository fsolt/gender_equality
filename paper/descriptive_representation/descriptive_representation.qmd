...
Italy was the pioneering among these countries, but its small quota was abolished a few years later.
...
With this dataset and model, this distribution indicates that there is a 90% chance that this variable has a positive effect on women's descriptive representation.
...
And if rising public gender egalitarianism coincides with, for example, the greater success of the green and left parties committed to equality, then the results just described may be the consequence of aggregation bias.
...
if (!file.exists(file.path(descrep_path,  "data", "pfem_results.rda"))) {
  ...
  m2pf_data_2sd <- m2pf_data %>% 
    ...
    mutate(`.variable` = case_when(name == "pge_mean" ~ 
                                      ...
                                      name == "pge_diff" ~
                                        "bsp_mepge_diffpge_diff_se",
                                      TRUE ~ paste0("b_", name)),
              var_names = c("Gender Quota, Difference",
  ...
  save(m2_data, m2_data_2sd, coef_data0_m2, cyp_summary,
...
Regardless, as in the previous analysis, we include information about the country-mean and the difference from this mean for the presence of a party list in the electoral system drawn from the the Democratic Electoral Systems Around the World dataset dataset [@Bormann2022].
...
coef_data_m2_total <- coef_data0_m2pf %>% 
  group_by(.chain, .iteration, .draw) %>% 
  mutate(alpha = mean(ifelse(name=="women_rep_lag", .value, NA_real_),
                      na.rm = TRUE)) %>% 
  ungroup() %>% 
...
Figure&nbsp;\ref{m2plot} shows the results of this model of women's share of seats within parties' parliamentary parties.
They reinforce the conclusions reached when looking only at the legislature as a whole.
...
Compared to right-wing parties, women's share of elected representatives is `r get_coef("green", coef_data_m2pf)` points higher in green parties and `r get_coef("left", coef_data_m2pf)` points higher in parties of the left.
Women's shares of parliamentary delegations of parties of the ideological center were found to be  `r get_coef("green", coef_data_m2pf)` points larger as well, but extreme right parties are no different from other parties of the right.

Most importantly for our purposes, focusing in on parties within elections and taking into account party ideology do not change our findings with regard to public opinion.
...
Parties of a country where public opinion is a standard deviation above the sample mean elect `r get_coef("pge_mean", coef_data_m2pf)` points more women representatives within their delegations than those in a country where public opinion is a standard deviation below.
And over time _within_ a country, a two-standard deviation increase in public gender egalitarianism yields a `r get_coef("pge_diff", coef_data_m2pf)` point increase in the extent to which parties send women to the legislature.
The conclusions reached in our analysis of entire legislative elections are not an artifact of aggregation.


## Addressing Reciprocity: Accounting for Representation's Effects on Opinion {.unnumbered}
