...
In ten of the OECD countries, no more than one in twenty members of the lower house of the national legislature were women into the 1980s or even later.
...
Case studies and cross-national work alike have found support for the elite-led political theory described in Chapter 2.
...
And the national quota laws advocated by these activists that, when adopted, require all parties to put forward a minimum share of women as candidates, likewise have been found generally to increase the share of women elected, if not typically by the amount their mandated candidate shares would perhaps lead one to expect [see, e.g., @Schwindt-Bayer2009; @Hughes2015].
...
To start, we will need to take into account other potential explanations for when more women are elected to office, in particular, those offered by the political theory.

public opinion may simply reflect cues provided by the activists who play a central role in the political theory.
If that is the case, both public opinion (through these cues) and descriptive representation (through their successes in getting national quotas enacted and women candidates on party lists) would be consequences of the strength of feminist activists.
...
```{r qarot, fig.cap="De Facto Threshold Adoption and Reform in OECD Democracies \\label{wrplot}", fig.height = 4.5, fig.width = 7.5}
dft_data <- by_country %>% 
  group_by(country) %>% 
...
That dataset defines the de facto legislative threshold on the basis of "a country's stated quota threshold and the breadth of a quota's actual reach" [@Hughes2019, 225].
...
However, this quota applies only to the proportion that is elected from these party lists, about a sixth of the National Assembly under the country's mixed-member electoral system.
...
Taking these quota thresholds and their respective breadths of application together, the de facto quota in Korea is about one-third of each party's candidates.
...
Still, the de facto threshold provides a straightforward measure of the share of women candidates that is required to appear on the ballot, and we use it as our measure of national quotas here.

With data in hand, we can turn to how best to use it to make inferences.
...
As in most time series data, that is true in this case.
...
In either event, the extent of women's descriptive representation in the preceding term can be expected to influence the extent of representation in this one.
...
```{r m1plot, fig.cap="Predicting Women's Parliamentary Representation in OECD Democratic Elections \\label{m1plot}", fig.height = 7, fig.width = 7.5}
...
To review the findings, we found plenty of support for the well established political theory that feminist activists working within political parties are a major force for getting women elected.
...
But there's strong support, too, for the bottom-up theory, which complicates the purest versions of the political theory: the activists working hard within parties to build space for women are not the entire story.
