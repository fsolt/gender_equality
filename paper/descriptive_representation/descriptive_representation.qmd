...
ggplot(data = by_country0, aes(x = year, y = women_rep)) +
    theme_bw() +
...
Its y-axis depicts the extent of women's descriptive representation, measured as the percentage of seats won by women in each election to the lower house of the national legislature in that election.
...
@Shor2007 demonstrates that two of these difficulties with pooled time series are best addressed using a Bayesian multilevel model that includes varying intercepts for both space and time.
...
if (!file.exists(file.path(descrep_path, "data", "wr_results.rda"))) {
  ...
  cy_summary <- m1_wr_b$data %>%
    count(country) %>%
    pull(n) %>%
    summary()
  
  save(m1_data_2sd, coef_data0, cy_summary, 
...
```{r resplot, fig.cap="Predicting Women's Parliamentary Representation in OECD Democratic Elections \\label{model}", fig.height = 9, fig.width = 7.5}
...
pfem_label <- tibble(pge = 30, pfem = 100, label = pfem_cor)
...
ggplot(by_party,
  ...
  labs(x = TeX("Public Gender Egalitarianism"), y = "Women's Share of Party Legislative Seats") +
