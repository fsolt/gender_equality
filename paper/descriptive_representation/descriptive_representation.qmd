...
As @Kim2023 [, 17] notes, "Perhaps the biggest concern is that preexisting egalitarianism skews results," a concern the study could not address in its model due to the limited observations available in the WVS data.
...
Fortunately, the dataset used in this chapter, built around the extensive PGE data on gender egalitarianism, allows us to proceed along exactly these lines.
We model the potentially reciprocal relationships using a multivariate simultaneous equations model.
This means that gender egalitarianism and descriptive representation are each used to predict the other and that the varying intercepts for each country and year are modeled as correlated across the two equations.
