...
The PGE data presented in the previous chapter provides us, for the first time, with some leverage to get answers. 
...
# QAROT: national quotas and women's representation
...
```{r wr_models_dynamic}
m0_wrd <- country_pge %>%
  map(~ lmer(women_rep ~ lag(women_rep) + pge_lag_by2sd +
               (1 | country), REML=FALSE,
             data = .x)) %>% 
  sim_and_tidy()
...
vars_proper_wrd <- c(bquote("Women's Seat Share"[t-1]),
                     bquote(PGE[t-1]),
                     "National Quota",
                     "Party List",
                     bquote(PGE[t-1] %*% National~Quota),
...
walk(1:5, function(mod) {
  m04_wrd %>% 
    ...
    theme(legend.position="none",
          ...
          axis.text  = element_text(size=12)) +
    geom_vline(xintercept = 0, colour = "grey60", linetype = 2) +
    coord_cartesian(xlim = c(min(m04_wrd$estimate - 1.96*m04_wrd$std.error), 
                             max(m04_wrd$estimate + 1.96*m04_wrd$std.error))) +
    scale_color_viridis_d(direction = -1,
                          end = .8) +
    xlab("Standardized Coefficients") +
    ggtitle("Multilevel Models of Women's Share of Legislative Seats")
  ggsave(here::here("paper",
                    "descriptive_representation",
                    paste0("m04_wrd_", mod, ".jpg")))
})

m04_wrd %>% 
  dwplot() +
  scale_y_discrete(labels=do.call(expression, rev(vars_proper_wrd))) +
  theme_bw() +
  theme(legend.position="none",
        plot.title.position = "plot") +
  geom_vline(xintercept = 0, colour = "grey60", linetype = 2) +
...
save(by_country2, file = file.path(descrep_path, "data", "by_country2.rda"))
save(m0_wr_b, file = file.path(descrep_path, "data", "m0_wr_b.rda"))
save(m2_wr_b, file = file.path(descrep_path, "data", "m2_wr_b.rda"))
```

```{r wr_b_interactions, eval=FALSE}
m3_wr_b <- brm(formula = bf(women_rep ~
                              me(pge_mean, pge_mean_se) +
                              me(pge_diff, pge_diff_se) +
                              national_quota_mean +
                              national_quota_diff +
                              list_mean +
                              list_diff +
                              me(pge_diff, pge_diff_se)*national_quota_mean +
                              (1 | country) + (1 | year)),
               data = by_country2,
               backend = "cmdstanr",
               warmup = 1000,
...
m3_wr_b1 <- brm(formula = bf(women_rep ~
                              me(pge_mean, pge_mean_se) +
                              me(pge_diff, pge_diff_se) +
                              national_quota_mean +
                              national_quota_diff +
                              list_mean +
                              list_diff +
                              me(pge_diff, pge_diff_se)*national_quota_diff +
                              me(pge_diff, pge_diff_se)*national_quota_mean +
                              (1 | country) + (1 | year)),
               data = by_country2,
               backend = "cmdstanr",
...
# m4_wr_b <- brm(formula = bf(women_rep ~
#                               me(pge_mean, pge_mean_se) +
#                               me(pge_diff, pge_diff_se) +
#                               national_quota_mean +
#                               national_quota_diff +
#                               list_mean +
#                               list_diff +
#                               me(pge_diff, pge_diff_se)*list_mean +
#                               (1 | country) + (1 | year)),
#                data = by_country2,
#                backend = "cmdstanr",
#                warmup = 500,
...
#                seed = 324)
# 
# m4_wr_b1 <- brm(formula = bf(women_rep ~
#                               me(pge_mean, pge_mean_se) +
#                               me(pge_diff, pge_diff_se) +
#                               national_quota_mean +
#                               national_quota_diff +
...
#                               list_diff +
#                               me(pge_diff, pge_diff_se)*list_mean +
#                               me(pge_diff, pge_diff_se)*list_diff +
#                               (1 | country) + (1 | year)),
#                data = by_country2,
#                backend = "cmdstanr",
#                warmup = 500,
...
save(doubled_sd, coef_data0, cy_summary, file = file.path(descrep_path,
                                                          "c_results.rda"))
} else {
  load(file = file.path(descrep_path, "c_results.rda"))
}
```


```{r resplot, fig.cap="Predicting Women's Parliamentary Representation in OECD Democracies \\label{model}", fig.height = 5.5, fig.width = 7.5}
...
parties %>% 
  ...
  scale_fill_distiller(na.value = "gray90", 
                       ...
                       direction = 1) +
  theme(legend.position="none")
ggsave(file.path(descrep_path, "map_oecd_party_elections.jpg"),
       width = 7, height = 3.5)
...
```{r pfem_models_dynamic}
m0 <- party_pge %>%
  map(~ lmer(pfem ~ lag(women_rep) + pge_lag_by2sd +
               (1 | party) + (1 | cyear) + (1 | country), REML=FALSE,
             data = .x)) %>% 
  sim_and_tidy()
...
vars_proper_d <- c(bquote("Women's Seat Share"[t-1]),
                   bquote(PGE[t-1]),
                   "Quota",
                   "Party List",
                   bquote(PGE[t-1] %*% Quota),
