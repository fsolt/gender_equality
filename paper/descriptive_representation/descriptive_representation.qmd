...
But the observations in the foregoing analysis are country-elections. 
At that level, we cannot really see what going on within political parties, not really at all.
The results just described therefore may be the consequence of aggregation bias, leading to mistaken conclusions due to all of each country's parties being combined at every election.
...
(Parties whose delegations in the lower house of the national legislature are mostly women tend to be smaller, but otherwise there is little apparent relationship between party size and descriptive representation (across all observations, $R$ = `r with(by_party2, cor(pfem, seat_percent)) %>% round(2)`.)
...
It also includes with varying intercepts the unobserved distinctive traits of each party, country-election, country, and year.
...
Figure&nbsp;\ref{m2plot} shows the results.
