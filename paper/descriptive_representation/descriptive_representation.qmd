...
if (!file.exists(file.path(descrep_path, "data", "wr_results.rda"))) {
  ...
  by_country <- by_country0 %>%
      ...
      left_join(act_summary %>% 
                    transmute(country, 
                              year,
                              act,
                              act_lag = lag(act),
                              act_se,
                              act_se_lag = lag(act_se)),
  ...
  by_country2 <- by_country %>% 
    ...
    transmute(year,
              ...
              act,
              act_se,
              act_lag,      # from previous year
              act_se_lag,
              act_mean = mean(act, na.rm = TRUE),
              act_mean_se = sqrt(sum(act_se^2, na.rm = TRUE))/
                  length(act),
              act_diff = act - act_mean,
              act_diff_se = sqrt(act_se^2 + act_mean_se^2)/2,
              national_quota,
  ...
  bf_wr <- bf(women_rep ~ 
                  ...
                  me(act_diff, act_diff_se) +
                  me(act_mean, act_mean_se) +
                  national_quota_diff +
                  national_quota_mean + 
                  list_diff +
                  list_mean +
  ...
  m1_data_2sd <- m1_data %>% 
    ...
    mutate(`.variable` = case_when(name == "pge_mean" ~ 
                                   ...
                                   name == "act_mean" ~ 
                                        "bsp_meact_meanact_mean_se",
                                      name == "act_diff" ~
                                        "bsp_meact_diffact_diff_se",
                                      TRUE ~ paste0("b_", name)),
              var_names = c("Public Gender Egalitarianism, Difference",
                            "Public Gender Egalitarianism, Mean",
                            "Women's Activism, Difference",
                            "Women's Activism, Mean",
...
We return to examining country-elections to match the level at which both public opinion descriptive representation can be measured.
