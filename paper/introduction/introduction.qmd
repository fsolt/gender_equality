...
cited <- tribble(~citation, ~text_x, ~text_y,
                 "Alexander 2012", 29, 2.4,
                 "Paxton and Kunovich 2003", 48, 1.7,
                 "Alesina et al. 2013", 78, 0.25,
                 "Scarborough et al. 2021", 4, 21.5,
                 "Fortin 2005", 28, 3.5)
...
gr <- read_csv(here("paper",
  ...
  # mutate(text_color = if_else(is.na(text_x), "white", "gray20"),
  #        text_x = if_else(is.na(text_x), k, text_x))
```

```{r gr-plot, fig.cap="Countries and Mean Years Observed in Prominent Research\\label{gr_plot}", fig.height=8, fig.width=7.5, fig.pos='h', cache=FALSE}

gr_alpha <- .3
my_palette <- colorRampPalette(brewer.pal(11, "Spectral"))
color_scale <- scale_color_gradientn(colors = my_palette(max(gr$year) -
                                                           min(gr$year) + 1),
                                     limits = c(min(gr$year),
                                                max(gr$year)),
                                     name="Publication\nYear")
fill_scale <- scale_fill_gradientn(colours = my_palette(max(gr$year) -
                                                          min(gr$year) + 1),
                                   limits = c(min(gr$year),
                                              max(gr$year)),
                                   name="Publication\nYear")
...
gr_plot <- ggplot(gr, aes(x = k,
  ...
  annotate(geom = "rect", 
           ...
           fill = NA,
           linewidth = .25 ) +
  geom_segment(aes(x = 0, y = 5, xend = 13, yend = 16.2),
               linetype = "dashed",
               linewidth = .25) +
  geom_segment(aes(x = 5, y = 0, xend = 30.5, yend = 4.7),
               linetype = "dashed",
               linewidth = .25) # +
  # geom_segment(aes(x = 46, y = 1, xend = 47.5, yend = 1.6),
  #              linetype = "solid",
  #              linewidth = .25,
  #              color = "gray20")
...
gr_plot + 
  inset_element(zoom_plot, 15/80, 5/20, 30/80, 15/20) +
  plot_annotation(caption = str_wrap("Notes: Citation counts as reported by the Web of Science on January 4, 2025.  See Appendix A for details on this sample of prominent articles.", width = 110))

```

Data availability has influenced scholarship on the topic, as can be seen in Figure\nobreakspace{}\ref{gr_plot}.
...
Another quarter of these articles look at change over time, examining two or more years of data within just one country.
The recent work of @Scarborough2021 employs U.S. General Social Survey data in 21 years from 1977 to 2018 to examine how gender and racial attitudes covaried over this period.
Over a third of the sample considers data from multiple countries in a single cross-section.
In the lower right corner, the much-cited work of @Alesina2013 collapses data from three waves of the World Values Survey on 79 countries to examine whether traditions of plow-based agriculture have a negative influence on the extent of egalitarian gender attitudes.
